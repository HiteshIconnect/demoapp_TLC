...
export const fetchWorkOrderDetails = (wo, token, email, role, soNo) => {
  ...
  return (dispatch, getState) => {
    ...
    dispatch(setLoader());
    api
      .getDataWithParams(sample, 'mainUrl', params, token, query)
      .then(res => {
        console.log('Response for SO fecth: ', res);
        dispatch(unsetLoader());
        if (
          res.data.Error ||
          res.data.Message !== 'WO details fetched successfully' ||
          res.data.Message !== 'WO details fetched successfully'
        ) {
          dispatch(
            setError(
              'Sorry its incorrect',
              'It Looks like you entered a wrong sales Order.',
              '',
            ),
          );
      ...
      .catch(err => {
        ...
        dispatch(
          setError(
            'Sorry its incorrect',
            'It Looks like something is wrong with the Server.',
            '',
          ),
        );
...
export const updateWODetails = (data, token) => {
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      dispatch(unsetLoader());
      console.log('error update wo', error);
