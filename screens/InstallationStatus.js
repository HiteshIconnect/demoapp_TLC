...
import { Text, View, Image, TextInput, TouchableOpacity, FlatList, BackHandler, StyleSheet, Dimensions, KeyboardAvoidingView, } from "react-native";
...
import {
  selectInstallList,
  imageUploaded,
} from "../redux/installation/selector";
...
const InstallationStatus = ({
  ...
  const backAction = () => {
    ...
    screen === "Installation Camera" ? navigation.navigate('DashBoard') : navigation.goBack()
  };
  ...
  return (
    <>
      ...
      >
        <Header
          navigation={navigation}
          title={`${screen === "Capture Images" || screen === "Installation Camera"
            ? "Capture Images"
            : status?.Status
            }`}
        />
        ...
        <View style={styles.container}>
          ...
          <FlatList
            style={{ width: "100%", marginTop: "5%", height: screen === "Capture Images" ? 410 : 298.5 }}
            data={tempList ? tempList : currentPosts}
            keyExtractor={item => item.id.toString()}
            renderItem={({ item }) => {
 
              return (
                <ListItem changeSelectedItem={changeSelectedItem} item={item} mode={mode} list={list} screen={screen} selectItem={selectItem} setList={setList} />
              );
            }}
          />
 
          <View style={styles.btnContainer}>
            <AppButton onPress={screen === "Installation Camera" ? updateDropbox : updateWO}
              isChecked={isChecked}
            />
          </View>
