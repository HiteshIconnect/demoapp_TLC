...
import {fetchWorkOrderDetails,fetchRecentWorkOrder, fetchRecentSalesOrder ,storeWorkOrderList } from '../redux/workOrder/actions';
import { postScannedBarcodes,allowUserToSeeWO } from '../redux/barcodeScanner/actions';
  ...
  allowUserToSeeWO
    ...
    const screenNavigator=async(id)=>{
      ...
      if(screen==="Work Order Details"){
        //await fetchWorkOrderDetails('',token,userDetails.email,userDetails.currentRole,id);
        await postScannedBarcodes([id], userDetails.currentRole, userDetails.email,token);
        //await changePoNum("")
      }
    ...
    useEffect(()=>{
      ...
      if(fetchStatus==true){
        ...
        if(screen==="Inventory Transfer") fetchRecentInventory(token,userDetails.email)
        if(screen=="Production Management") fetchRecentSalesOrder(token,userDetails.email)
        if(screen==="InstallationDeliver" || screen === 'Pick Up' || screen === "Capture Images" || screen === "Delivered to Client"|| screen==="Work Order Details") fetchRecentWorkOrder(token,userDetails.email)
      }
      ...
      if(showWO){
        if(screen==="Work Order Details"){
          ...
          return dataList.push({
            ...
            navigation.navigate(screenMap[screen],{itemDetails:dataList[0],prodStatus:"abcdefg",moveTo:"",screen:screen})
            updateWos([]);
    ...
    const fetchPODetails = async (PO) => {
      ...
      if(screen==="InstallationDeliver" || screen==="Pick Up" || screen === "Capture Images" || screen === "Delivered to Client"){
        ...
        if(woArray.length===1){
          ...
          await postScannedBarcodes(woArray, userDetails?.currentRole, userDetails?.email,token);
          changePoNum('');
          return;
        }
    ...
    const openScanner=()=>{
      ...
      if(screen==="InstallationDeliver" || screen === 'Pick Up' || screen === "Capture Images" || screen === "Delivered to Client" || screen==="Work Order Details"){
    ...
    return (
      ...
      <ErrorModal />
        ...
        />
        <CaptureImageModal searchMore={searchWorkOrders}  mode={'multiple search'}/>
          ...
          <View style={purchaseOrderStyle.children}>
            ...
            <View style={purchaseOrderStyle.barcodeContainer}>
              <View style={purchaseOrderStyle.barcode}>
                <View style={purchaseOrderStyle.barcodeFlex}>
                  ...
                  <TouchableOpacity
                    style={{justifyContent:'center',alignContent:"flex-end",marginEnd:2}}
                    onPress={openScanner}>
            ...
            <View style={purchaseOrderStyle.loadOrderContainer}>
              <TouchableOpacity
                ...
                style={poNum ||Boolean(wos.length>0)?purchaseOrderStyle.loadOrderButton:purchaseOrderStyle.loadOrderButtonDis}
